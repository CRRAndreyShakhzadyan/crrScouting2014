...
function fmsDecode($tweet)//remember to change the regex pattern match to amend what fms standard changes
{
	...
	if($result =! false)
	{
		preg_match('/#FRC([A-Za-z0-9]*) TY ([PQE]) MC ([0-9]{1,3}) RF ([0-9]{1,3}) BF ([0-9]{1,3}) RA ([0-9]{1,4}) ([0-9]{1,4}) ([0-9]{1,4}) (BA) ([0-9]{1,4}) ([0-9]{1,4}) ([0-9]{1,4}) (RC) ([0-9]{1,2}) (BC) ([0-9]{1,2}) (RFP) ([0-9]{1,2}) BFP ([0-9]{1,2}) RAS ([0-9]{1,2}) BAS ([0-9]{1,2}) RTS ([0-9]{1,3}) BTS ([0-9]{1,3})//',$tweet, $name_buffer, NULL);
	}
	print_r ($names_buffer);
	// if(is_array($result))
	// {
		for($iii;$iii < count($result);$iii++)
		{
			if($iii =! 0)
			{
				if($iii == 1)
				...
				$outputarray[$names_buffer[($iii)]] = $result[$iii];
			}
		}
	// }
	return $outputarray;
